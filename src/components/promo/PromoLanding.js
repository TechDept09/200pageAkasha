...
// Promotional landing surface. Rendered directly at `/` by
// pages/index.js against the active promo config (see lib/promo.js).
// No auth gate here, no phase override query, no preview route — the
// preview flow now lives on localhost + cloudflared tunnel, so the
// legacy /campaign/* gating and phase-picker have been removed.
export default function PromoLanding({ phase }) {
  ...
  return (
    <>
      <Head>
        ...
        {process.env.NEXT_PUBLIC_PROMO_NOINDEX === 'true' ? (
      ...
      {/* Page-wide mandala backdrop. Fixed to the viewport with a low
          z-index so it stays pinned across every section. Sections
          with opaque backgrounds cover it; sections with transparent
          or semi-transparent backgrounds let it show through. */}
      <div
        aria-hidden="true"
      ...
      <main className="relative z-[1] bg-transparent pb-24 md:pb-20 overflow-x-hidden">
        ...
        {/* Meet-the-teachers grid: Burkhard, Kirsten, Devdas photos +
            bios. Puts real faces to the training right after the
            certification claim so trust builds before the pitch. */}
        <Teachers />
        ...
        {isBackup ? (
          ...
          <>
            ...
            <VideoTestimonial
              videoId="gky-DlxSFgE"
              heading="Hear It From Our Graduates"
              description="Real stories from real students who transformed their practice through Akasha."
            />
...
function TestimonialCarousel() {
  ...
  return (
    <section
      className="py-16 md:py-24 bg-akasha-gold/5"
